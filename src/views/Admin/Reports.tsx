...
import { BarChart3, TrendingUp, Package, Calendar, ArrowUpRight } from 'lucide-react';
  ...
  return (
    <div className="animate-slide-up pb-10" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div className="flex justify-between items-end">
        <div>
          <h3 style={{ fontSize: '0.75rem', fontWeight: 800, color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '4px' }}>Analytics Overview</h3>
      ...
      <div style={{ background: '#F0F9FF', padding: '1.5rem', borderRadius: '24px', border: '1px solid #E0F2FE' }}>
        ...
        <p style={{ fontSize: '0.9rem', color: '#1E40AF', lineHeight: '1.6', fontWeight: 500, margin: 0 }}>
          Your sales are currently driven by <span style={{ fontWeight: 800 }}>{productSales[0]?.name || 'N/A'}</span>. 
          The average order value is <span style={{ fontWeight: 800 }}>₱{stats.avgOrderValue.toFixed(0)}</span>, 
          which is healthy for your current scale.
        </p>
      </div>
    </div>
  );
...
const MiniStat = ({ label, value, icon, color }: { label: string, value: string, icon: React.ReactNode, color: string }) => (
  <div style={{ background: 'white', padding: '1rem', borderRadius: '24px', border: '1px solid #F3F4F6', display: 'flex', flexDirection: 'column', gap: '8px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)' }}>
    <div style={{ color: color, background: `${color}15`, width: '32px', height: '32px', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      {icon}
    </div>
    <div>
